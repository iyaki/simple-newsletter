...
test('createConfirmation returns SubscriptionConfirmation with correct recipient and confirmation URI contains token', function () {
...
test('createNewsletter returns Newsletter with correct subject format and cancellation URI contains token', function () {
    $factory = new EmailTemplateFactory('https://example.com');
    $feed = new Feed('https://example.com/feed', 'Test Feed', 'https://example.com', new \DateTimeImmutable());
    $subscription = new Subscription('https://example.com/feed', 'user@example.com');
